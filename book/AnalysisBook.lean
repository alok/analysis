...
def theme : Theme :=
  ...
  let passthrough : Template := do
    return (← param "content")
  let frontpage : Template.Override :=
    { template := do return {{<div class="frontpage"><h1>{{← param "title"}}</h1> {{← param "content"}}</div>}}
    , params := id
    }
  { primaryTemplate := primary
    pageTemplate := passthrough
    postTemplate := passthrough
    archiveEntryTemplate := passthrough
    categoryTemplate := passthrough
    adHocTemplates := fun path => if path = #[] then some frontpage else none
  }
...
def blogMain (theme : Theme) (siteSpec : Site)
    (linkTargets : Code.LinkTargets TraverseContext := {})
    (options : List String) : IO UInt32 := do
  ...
  let (siteOut, xref) ← siteSpec.traverse cfg components
  let ctx : Generate.Context := {
    «site» := siteOut
    ctxt := {path := .root, config := cfg, components := components}
    xref := xref
    linkTargets := linkTargets
    dir := cfg.destination
    config := cfg
    header := Verso.Output.Html.doctype
    components := components
  }
  let _ ← (siteOut.generate theme).run ctx |>.run {} |>.run {}
