...
Technical point: in the text, the functions {lit}``f`` studied are defined only on subsets {lit}``X`` of {lean}``ℝ``, and
left undefined elsewhere. However, in Lean, this then creates some fiddly conversions when trying
to restrict {lit}``f`` to various subsets of {lit}``X`` (which, technically, are not precisely subsets of {lean}``ℝ``,
