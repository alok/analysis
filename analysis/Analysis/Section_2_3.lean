...
Note: at the end of this chapter, the {lit}``Chapter2.Nat`` class will be deprecated in favor of the
standard Mathlib class {lit}``_root_.Nat``, or {lit}``ℕ``. However, we will develop the properties of
{lit}``Chapter2.Nat`` "by hand" for pedagogical purposes.
...
/-- This instance allows for the {lit}``*`` notation to be used for natural number multiplication. -/
instance Nat.instMul : Mul Nat where
  mul := mul

/-- Definition 2.3.1 (Multiplication of natural numbers)
Compare with Mathlib's {lit}``Nat.zero_mul`` -/
theorem Nat.zero_mul (m: Nat) : 0 * m = 0 := recurse_zero (fun _ prod ↦ prod+m) _

/-- Definition 2.3.1 (Multiplication of natural numbers)
Compare with Mathlib's {lit}``Nat.succ_mul`` -/
...
/-- Compare with Mathlib's {lit}``Nat.one_mul`` -/
...
/-- Lemma 2.3.3 (Positive natural numbers have no zero divisors) / Exercise 2.3.2.
    Compare with Mathlib's {lit}``Nat.mul_eq_zero``. -/
lemma Nat.mul_eq_zero (n m: Nat) : n * m = 0 ↔ n = 0 ∨ m = 0 := by
  sorry

/-- Proposition 2.3.4 (Distributive law)
Compare with Mathlib's {lit}``Nat.mul_add`` -/
...
/-- Proposition 2.3.4 (Distributive law)
Compare with Mathlib's {lit}``Nat.add_mul`` -/
...
Compare with Mathlib's {lit}``Nat.mul_assoc`` -/
theorem Nat.mul_assoc (a b c: Nat) : (a * b) * c = a * (b * c) := by
  sorry

/-- (Not from textbook)  Nat is a commutative semiring.
    This allows tactics such as {lit}``ring`` to apply to the Chapter 2 natural numbers. -/
...
/-- This illustration of the {lit}``ring`` tactic is not from the
    textbook. -/
example (a b c d:ℕ) : (a+b)*1*(c+d) = d*b+a*c+c*b+a*d+0 := by ring


/-- Proposition 2.3.6 (Multiplication preserves order)
Compare with Mathlib's {lit}``Nat.mul_lt_mul_of_pos_right`` -/
...
/-- Proposition 2.3.6 (Multiplication preserves order)
Compare with Mathlib's {lit}``Nat.mul_lt_mul_of_pos_left`` -/
...
Compare with Mathlib's {lit}``Nat.mul_right_cancel`` -/
...
This allows tactics such as {lit}``gcongr`` to apply to the Chapter 2 natural numbers. -/
...
Compare with Mathlib's {lit}``Nat.mod_eq_iff`` -/
...
/-- Definition 2.3.11 (Exponentiation for natural numbers)
Compare with Mathlib's {lit}``Nat.pow_zero`` -/
@[simp]
...
/-- Definition 2.3.11 (Exponentiation for natural numbers)
Compare with Mathlib's {lit}``Nat.pow_succ`` -/
theorem Nat.pow_succ (m n: Nat) : (m:Nat) ^ n++ = m^n * m :=
  recurse_succ (fun _ prod ↦ prod * m) _ _

/-- Compare with Mathlib's {lit}``Nat.pow_one`` -/
@[simp]
