...
/-- A cast is needed here because {lit}``(d₁+d₂)+d₃`` is not definitionally equal to {lit}``d₁+(d₂+d₃)``. -/
...
/-- A cast is needed here because {lit}``2 • d`` is not definitionally equal to {lit}``d+d``. -/
